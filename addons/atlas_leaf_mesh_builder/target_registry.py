"""Durable Atlas Leaf target-SPM registry shared with external batch tools."""
...
def _safe_remove_temporary(path):
    for attempt in range(REPLACE_MAX_ATTEMPTS):
        try:
            ...
            if attempt + 1 < REPLACE_MAX_ATTEMPTS:
                time.sleep(REPLACE_RETRY_DELAY_SECONDS)
    return False


def _replace_registry_file(temporary, destination):
    original_error = None
    for attempt in range(REPLACE_MAX_ATTEMPTS):
        try:
            ...
            if original_error is None:
                original_error = exc
            if _same_file_bytes(destination, temporary):
...
def save_target_registry(blend_path, target_spms):
    ...
    _replace_registry_file(temporary, registry_path)
    payload["registry_path"] = str(registry_path)
    return payload
